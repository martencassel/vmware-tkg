...
# Setup CLI
#
get-tkg-files.sh $TKG_VERSION
get-tanzu-cli.sh $TKG_VERSION
get-tkg-files.sh $TKG_VERSION

get-tkg-file.sh $TKG_VERSION "$KUBECTL_FILE.gz" $KUBECTL_FILE
...
if [ ! -z "$OVA_FILE" ]; then
    get-tkg-files.sh $TKG_VERSION
    get-tkg-file.sh $TKG_VERSION $OVA_FILE $OVA_FILE
